 ...
 * Updated: Added Task ID badge display at the beginning of each task card
 */
"use client"
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              <Badge variant="default" size="sm" className="bg-white/20 text-white/90 font-mono border border-white/20">
                #{task.id}
              </Badge>
              <h3
              ...
              {task.category && (
                <Badge variant="default" size="sm" className="bg-white/20 text-white/90 border border-white/20">
                  📁 {task.category}
                </Badge>
              )}
            </div>
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            <Button
              ...
              className="h-10 w-10 sm:h-8 sm:w-8 p-0 hover:bg-white/10"
              title={task.completed ? "Mark incomplete" : "Mark complete"}
            >
              {task.completed ? (
                <X className="h-5 w-5 sm:h-4 sm:w-4 text-white/70" />
              ) : (
                <Check className="h-5 w-5 sm:h-4 sm:w-4 text-[#10B981]" />
              )}
            </Button>
            <Button
              variant="ghost"
