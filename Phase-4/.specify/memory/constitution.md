...
## Interaction Model
- Users may interact with the system via UI or conversation.
- Conversational interaction must support all existing task features.
- The system must remain usable without conversational features failing core functionality.

## AI & Tooling Rules
- AI agents must not directly access the database or business logic.
- All task operations must be performed through exposed tools.
- Tools must be stateless and persist state only via the database.
- AI agents must remain stateless between requests.

## MCP Architecture
- Task operations must be exposed through an MCP server.
- MCP server must use the Official MCP SDK.
- Each task action must have a dedicated tool (e.g., add, update, delete).
- MCP server must remain independent from the UI and agent layers.

## State Management
- Chat endpoints must be stateless.
- Conversation state must be persisted in the database.
- No in-memory session state is allowed for conversations or agents.
...
- Keep clear separation between frontend, backend, AI agents, and MCP server.

## Governance
- All new features must comply with this constitution.
- If any specification or plan conflicts with this document, this document takes precedence.

## Core Principles (Phase I & II Legacy)
...
**Rationale**: The app evolves through phases (Console → Full-Stack → Conversational AI). Each phase adds measured complexity to deliver value while maintaining architectural integrity.
